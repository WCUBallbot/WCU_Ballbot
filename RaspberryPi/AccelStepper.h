...
/// \version 1.60 2019-01-22
///                Author: Brandon Beiler (BallBot)
///                Modified the AccelStepper code in order to facilitate running on a RaspberryPi platform. This included
///                the addition of a few libraries, a GPIOClass system for updating pins, and three new functions to 
///                work with the RaspberryPi pins instead of the Ardiuno pins
///
...
public:
	...
	/// This acts as a mapping between the integer number of the pin, and the actual GPIO pin object
	/// that can be acted upon. When a GPIO pin is registered, it is added to the map. This allows for 
	/// both retrieval of registered pins as well as a check to see if a pin has been registered yet
	unordered_map<int, GPIOClass*> pinMap;
    ...
    AccelStepper(int interface = AccelStepper::FULL4WIRE, int pin1 = 2, int pin2 = 3, int pin3 = 4, int pin4 = 5, bool enable = true);
    ...
    /// If the enable Pin is defined, sets it to 
	/// mode and sets the pin to enabled.
...
protected:
    ...
    virtual void   setOutputPins(int mask);
	...
	/// This method must be used to both register a GPIO pin as well as to set whether the pin will export voltages, 
	/// or read in voltages. 
	/// \param[in] pin the numeric value of the pin to change
	/// \param[in] dir a string value of either "out" or "in" depending on the direction of the signal for the pin
	void pinMode(int pin, std::string dir);

	/// This method uses the pin number provided to look up a registered GPIO pin on the raspberryPi. 
	/// If a registered pin is found, it will set the value of the pin to the corresponding int value. 
	/// The int value corresponds to the global LOW, HIGH fields
	/// \param[in] pin the numeric value of the pin to change
	/// \param[in] value the global LOW or HIGH variable, representing whether voltage will be output or not.
	void digitalWrite(int pin, int value);

	/// This method is used to unregister all GPIO pins that have been registered during the running of 
	/// the program. This is necessary to do at the end of all programs so that pins aren't left running 
	/// on the board.
	void closeAllPins();
...
private:
    ...
    int        _interface;          // 0, 1, 2, 4, 8, See MotorInterfaceType
